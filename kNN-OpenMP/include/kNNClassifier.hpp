...
{
    const auto testingDataSize = testingData.size();
    int rowIndex{};

    #pragma omp parallel for private(rowIndex) schedule(dynamic)
    for (rowIndex = 0u; rowIndex < testingDataSize; ++rowIndex)
    {
        ...
        {
            const auto distance = calculateDistance(trainRow, testingData[rowIndex]);
        ...
        testingData[rowIndex].predictedLabel = nearestLabel;
    }
}
...
void kNNClassifier<T>::predict(FlatDataView& testingData)
{
    ...
    for (rowIndex = 0u; rowIndex < testingDataSize; ++rowIndex)
    {
        ...
        {
            const auto distance = calculateDistance(trainingData[j], testingData[rowIndex]);
            if (distance < smallestDistance)
            {
                smallestDistance = distance;
        ...
        testingData[rowIndex].setPredictedLabel(nearestLabel);
    }
}
