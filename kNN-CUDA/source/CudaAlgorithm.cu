...
void Cuda::knn(const FlatDataView& trainingData, FlatDataView& testingData)
{
    ...
    const auto NUM_OF_BLOCKS = static_cast<uint32_t>(std::ceil(testingData.getNumberOfRows() / NUM_OF_THREADS) + 1);
    ...
    knnOnCuda<<<NUM_OF_BLOCKS, NUM_OF_THREADS>>>(deviceTrainingData, deviceTestingData, trainingData.getNumberOfRows(), testingData.getNumberOfRows(), testingData.getRowSize());
...
{
    constexpr int NUM_OF_THREADS = 1024;
    const auto NUM_OF_BLOCKS = static_cast<uint32_t>(std::ceil(data.getNumberOfRows() / NUM_OF_THREADS) + 1);
    auto& rawData = *data;
    ...
    findMinMaxOnCuda<<<NUM_OF_BLOCKS, NUM_OF_THREADS>>>(dev_rows, dev_minMax, data.getRowSize(), data.getNumberOfRows());
    minMaxNormalizationOnCUDA<<<NUM_OF_BLOCKS, NUM_OF_THREADS>>>(dev_rows, dev_minMax, data.getRowSize(), data.getNumberOfRows());
