...
FlatDataView DataDistributor::distributeTrainingData(const FlatDataView& data)
{
	...
	MPI_Bcast(dataBuffer.data(), dataSizeBuffer, MPI_FLOAT, MPI::MASTER_PROCESS, MPI_COMM_WORLD);
...
FlatDataView DataDistributor::distributeTestingData(const FlatDataView& data)
{
	...
	MPI_Scatter(rawData.data(), elementsPerProcBuffer, MPI_FLOAT, dataBuffer.data(), 
		elementsPerProcBuffer, MPI_FLOAT, MPI::MASTER_PROCESS, MPI_COMM_WORLD);
...
FlatDataView DataDistributor::collectData(const FlatDataView& data)
{
	...
	for (int i = procBeginData; i < procEndData; i++)
	{
		processBuffer.push_back(rawData[i]);
	}

	MPI_Gather(processBuffer.data(), processBuffer.size(), MPI_FLOAT, dataBuffer.data(), processBuffer.size(), MPI_FLOAT, MPI::MASTER_PROCESS, MPI_COMM_WORLD);
