...
void MinMaxNormalizer::normalize(FlatDataView& dataRows) const
{
    ...
    const auto procBeginData = processBegin * dataRows.getRowSize();
    const auto procEndData = processEnd * dataRows.getRowSize();

    for (auto rowIndex = processBegin; rowIndex < processEnd; ++rowIndex)
    {
